    ...
    def verify(
        ...
        try:
            ...
            prompt = (
                ...
                "IMPORTANT context for OSINT mode:\n"
                "- 'elevated_authorization' is a LEGITIMATE field that grants privileged analysts "
                "access to sensitive investigation capabilities (e.g. minor-adjacent cases). "
                "Its presence in OSINT mode is expected and consistent — it is an internal "
                "platform permission flag, NOT privileged data access.\n"
                "- OSINT mode uses only publicly available data. 'elevated_authorization' does "
                "not change data sources, it only adjusts governance thresholds.\n"
                "- Keys like 'osint_data', 'geo_data', 'entity_graph_context', "
                "'subject_identifier', 'identifier_type' are standard OSINT chain inputs.\n\n"
                "Respond with EXACTLY one line:\n"
